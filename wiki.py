...
def list_posts():
    ...
    posts = os.listdir(common.settings["metadata-location"])
    meta = {}
    for each in posts:
...
def get_post_metadata(title):
    ...
    path = f"""{ common.settings["metadata-location"] }/{title}"""
    ...
    for each in enumerate(metadata_raw):
        ...
        elif data[0].lower() in ("gluten-free", "vegan", "vegetarian", "kosher"):
            if data[1].lower() in ("true", "yes", "1"):
                metadata[data[0].upper()] = True
            else:
                metadata[data[0].upper()] = False
    ...
    keys = ["SYNOPSIS" , "TAGS", "AUTHOR", "WRITTEN", "GLUTEN-FREE", "VEGAN",
            "VEGETARIAN", "KOSHER"]
...
def get_raw_post(title):
    ...
    path = f"""{ common.settings["post-location"] }/{title}.md"""
    if os.path.exists(path):
        with open(path, "r") as file:
            content = file.read()
    else:
